...
- [ ] Нет длинного тире `—` в прозе и других ИИ-штампов (см. раздел 10)?
...
## 10. Анти-ИИ: чтобы текст не читался как сгенерированный

Рекрутеры (и всё чаще ATS) на глаз узнают «иишный» текст. Как только резюме или кейс пахнет генерацией — доверие падает, даже если по фактам всё хорошо. Главные палева:

- **Длинное тире `—` в прозе. Самый явный маркер.** Живые люди в резюме почти не ставят em-dash, а модели сыплют им в каждом втором предложении. Вместо него — точка, запятая или двоеточие. Тире допустимо только в датах (`2023 — 2024`) и в связке «Имя — Должность».
- **Идеальные триады и параллелизм:** «fast, scalable, and maintainable», «проектирую, строю и оптимизирую». Перечисления ровно из трёх с одинаковой грамматикой — типичный ИИ-ритм. Ломай симметрию: два пункта или четыре, разной длины.
- **Оборот «not just X, but Y» / «не просто X, а Y».** Любимая конструкция моделей. Переписывай прямо, без противопоставления.
- **Дежурные усилители и баззворды:** `seamless`, `robust`, `cutting-edge`, `leverage`, `delve`, `boasts`, `in today's fast-paced world`, «отказоустойчивый и масштабируемый» через запятую без единой конкретики за ними.
- **Стерильная гладкость.** Все предложения одной длины и структуры. У живого текста рваный ритм: короткое. Потом длиннее, с конкретной деталью и числом.
- **Пустые хвосты-обобщения:** `ensuring success`, `delivering value`, `что гарантирует надёжность и эффективность`. Обрубай — они не несут информации.

> Тест: прочитай вслух. Если звучит как пресс-релиз или маркетинговый лендинг, а не как живой инженер, объясняющий что и зачем он сделал — переписывай. Чуть проще и человечнее всегда лучше, чем гладко и безлико.

---

*Принцип одной строкой: показывай impact с числом, начинай с глагола, режь воду, пиши как человек — доверяй работе.*
