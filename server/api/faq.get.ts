const FAQ = {
  en: [
    { q: 'How do you approach system scalability?', a: 'I design for growth from day one — microservices, event-driven processing (Kafka), and layered caching (Redis/PostgreSQL) so the system stays responsive as load climbs.' },
    ...
    { q: 'What is your communication style during the development lifecycle?', a: 'I keep it transparent and regular — progress, architectural decisions, and risks — so we stay aligned with the business throughout.' }
  ],
  ru: [
    { q: 'Как вы обеспечиваете масштабируемость системы?', a: 'Проектирую с запасом на рост с первого дня: микросервисы, event-driven обработка (Kafka) и слоистое кэширование (Redis/PostgreSQL) — чтобы система оставалась отзывчивой по мере роста нагрузки.' },
    ...
    { q: 'Как вы работаете с техническим долгом?', a: 'Отношусь к техдолгу как к компромиссу, а не греху. Фиксирую его и гашу поэтапным рефакторингом по ходу разработки — код остаётся поддерживаемым, а выпуск фич не встаёт.' },
    { q: 'Вы работаете один или в команде?', a: 'И так, и так. Соло — вёл полный жизненный цикл продукта как единственный инженер. В команде — быстро вливаюсь в процессы и выдаю результат без постоянного контроля.' },
    ...
    { q: 'Как выстроена коммуникация в процессе разработки?', a: 'Держу коммуникацию прозрачной и регулярной: прогресс, архитектурные решения и риски — чтобы мы оставались в одной плоскости с бизнесом.' }
  ],
  hy: [
